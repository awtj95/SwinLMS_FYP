...
if($page=='add'){
	...
	$stmt = $db->prepare("update users set login_id=?, password=?,type=?, age=?, email=?, first_name=?, last_name=? , address=?, city=?, country=?, postcode=?, phone=?, contact=?
						, department=?, dob=?, photo=? , egname=?, egemail=?, egcontact=?, relationship=? where id=?");
	$stmt->bindParam(1,$login_id);
	$stmt->bindParam(2,$password);
	$stmt->bindParam(3,$type);	
	...
	$stmt = $db->prepare("insert into relation values('',?,?)");
	...
	$stmt = $db->prepare("select * from users where type='parent' order by id asc  ");
	$stmt->execute();
	$i = 1;
	while($row = $stmt->fetch()){
		...
		<tr>
			...
			<td>
			<button class="btn btn-info" data-toggle="modal" data-target="#link">Link</button>
			<!--Link Modal -->
			<div class="modal fade" id="link" tabindex="-1" role="dialog" aria-labelledby="addLabel">
			  <div class="modal-dialog" role="document">
				<div class="modal-content">
				  <div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					<h4 class="modal-title" id="addLabel">Link</h4>
				  </div>
				  <form>
				  <div class="modal-body">
					  <div class="form-group">
						<label for="u1">Parent:</label>
						<input type="text" class="form-control" id="u1" value="<?php echo $row['id'] ?>">
					  </div>
					  <div class="form-group">
						<label for="u2">Student:</label>
						<input type="text" class="form-control" id="u2" placeholder="">
					  </div>
				  </div>
				  <div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
					<button type="submit" onclick="linkData()" class="btn btn-primary">Save</button>
				  </div>
				  </div>
				  </form>
			  </div>
